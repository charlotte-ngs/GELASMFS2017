...
\subsection{R-Code zur Berechnung der
GRM}\label{r-code-zur-berechnung-der-grm}

Im Kapitel 13 von \citep{CV2013} ist ein R-Programm zur Berechnung der
genomischen Verwandtschaftsmatrix aufgeführt. Wir wollen dieses Programm
analysieren und präsentieren auch eine alternative Art der Berechnung
der genomischen Verwandtschaftsmatrix.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Making the genomic relationship matrix}
\NormalTok{nmarkers <-}\StringTok{ }\DecValTok{1000}
\CommentTok{# Reading SNP-Genotype codes (0,1,2) from file}
\NormalTok{data <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{scan}\NormalTok{(}\StringTok{"genotypes.txt"}\NormalTok{),}\DataTypeTok{ncol =} \NormalTok{nmarkers,}\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# Initialisation of variables}
\NormalTok{sumpq =}\StringTok{ }\DecValTok{0}
\NormalTok{freq =}\StringTok{ }\KeywordTok{dim}\NormalTok{(data)[}\DecValTok{1}\NormalTok{]}
\NormalTok{P =}\StringTok{ }\NormalTok{freq}
\NormalTok{lamda =}\StringTok{ }\KeywordTok{ncol}\NormalTok{(data)}
\CommentTok{# Compute allele frequencies and column vectors of matrix P}
\NormalTok{for(i in }\DecValTok{1}\NormalTok{:}\KeywordTok{ncol}\NormalTok{(data))\{}
  \NormalTok{(freq[i]  <-}\StringTok{ }\NormalTok{((}\KeywordTok{mean}\NormalTok{(data[,i])/}\DecValTok{2}\NormalTok{)))}
  \NormalTok{(P[i] =}\StringTok{ }\NormalTok{(}\DecValTok{2}\NormalTok{*(freq[i]-}\FloatTok{0.5}\NormalTok{)))}
  \NormalTok{(}\DataTypeTok{sumpq =} \NormalTok{sumpq+(freq[i]*(}\DecValTok{1}\NormalTok{-freq[i])))}
\NormalTok{\}}
\CommentTok{# Transform coding from (0,1,2) to (-1,0,1) and subtract P}
\NormalTok{Z <-}\StringTok{ }\NormalTok{data}
\NormalTok{for(i in }\DecValTok{1}\NormalTok{:}\KeywordTok{nrow}\NormalTok{(data))\{}
  \NormalTok{for(j in }\DecValTok{1}\NormalTok{:}\KeywordTok{ncol}\NormalTok{(data))\{}
    \NormalTok{(Z[i,j] <-}\StringTok{ }\NormalTok{((data[i,j]-}\DecValTok{1}\NormalTok{)-(P[j])))}
  \NormalTok{\}}
\NormalTok{\}}
\CommentTok{# Compute GRM}
\NormalTok{Zt =}\StringTok{ }\KeywordTok{t}\NormalTok{(Z)}
\NormalTok{ZtZ =}\StringTok{ }\NormalTok{Z%*%Zt}
\NormalTok{G =}\StringTok{ }\NormalTok{ZtZ/(}\DecValTok{2}\NormalTok{*sumpq)}
\NormalTok{G}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Reading SNP-Genotype codes (0,1,2) from file}
\NormalTok{data <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{scan}\NormalTok{(}\StringTok{"genotypes.txt"}\NormalTok{),}\DataTypeTok{ncol =} \NormalTok{nmarkers,}\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# Compute allele frequencies, column vectors of matrix P and sum of frequency products}
\NormalTok{freq <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(data, }\DecValTok{2}\NormalTok{, mean) /}\StringTok{ }\DecValTok{2}
\NormalTok{P <-}\StringTok{ }\DecValTok{2} \NormalTok{*}\StringTok{ }\NormalTok{(freq -}\StringTok{ }\FloatTok{0.5}\NormalTok{)}
\NormalTok{sumpq <-}\StringTok{ }\KeywordTok{sum}\NormalTok{(freq*(}\DecValTok{1}\NormalTok{-freq))}
\CommentTok{# Compute matrix Z by first changing the coding from (0,1,2) }
\CommentTok{# to (-1,0,1) and then subtracting matrix P}
\NormalTok{Z <-}\StringTok{ }\NormalTok{data -}\StringTok{ }\DecValTok{1} \NormalTok{-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(P, }\DataTypeTok{nrow =} \KeywordTok{nrow}\NormalTok{(data), }\DataTypeTok{ncol =} \KeywordTok{ncol}\NormalTok{(data), }\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# Z%*%Zt is replaced by tcrossprod(Z)}
\NormalTok{(matGrm <-}\StringTok{ }\KeywordTok{tcrossprod}\NormalTok{(Z)/(}\DecValTok{2}\NormalTok{*sumpq))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#' Compute genomic relationship matrix based on data matrix}
\NormalTok{computeMatGrm <-}\StringTok{ }\NormalTok{function(pmatData) \{}
  \CommentTok{# Allele frequencies, column vector of P and sum of frequency products}
  \NormalTok{freq <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(pmatData, }\DecValTok{2}\NormalTok{, mean) /}\StringTok{ }\DecValTok{2}
  \NormalTok{P <-}\StringTok{ }\DecValTok{2} \NormalTok{*}\StringTok{ }\NormalTok{(freq -}\StringTok{ }\FloatTok{0.5}\NormalTok{)}
  \NormalTok{sumpq <-}\StringTok{ }\KeywordTok{sum}\NormalTok{(freq*(}\DecValTok{1}\NormalTok{-freq))}
  \CommentTok{# Changing the coding from (0,1,2) to (-1,0,1) and subtract matrix P}
  \NormalTok{Z <-}\StringTok{ }\NormalTok{data -}\StringTok{ }\DecValTok{1} \NormalTok{-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(P, }\DataTypeTok{nrow =} \KeywordTok{nrow}\NormalTok{(pmatData), }
                         \DataTypeTok{ncol =} \KeywordTok{ncol}\NormalTok{(pmatData), }
                         \DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}
  \CommentTok{# Z%*%Zt is replaced by tcrossprod(Z)}
  \KeywordTok{return}\NormalTok{(}\KeywordTok{tcrossprod}\NormalTok{(Z)/(}\DecValTok{2}\NormalTok{*sumpq))}
\NormalTok{\}}

\CommentTok{# Computing the genomic relationship matrix using the above defined function}
\NormalTok{nmarkers <-}\StringTok{ }\DecValTok{1000}
\CommentTok{# Reading SNP-Genotype codes (0,1,2) from file}
\NormalTok{data <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{scan}\NormalTok{(}\StringTok{"genotypes.txt"}\NormalTok{),}\DataTypeTok{ncol =} \NormalTok{nmarkers,}\DataTypeTok{byrow =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# calling the function}
\NormalTok{(matGrmFunc <-}\StringTok{ }\KeywordTok{computeMatGrm}\NormalTok{(}\DataTypeTok{pmatData =} \NormalTok{data))}
\end{Highlighting}
\end{Shaded}
